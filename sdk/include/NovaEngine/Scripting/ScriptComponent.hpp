...
public:
    ...
    nlohmann::json properties     = nlohmann::json::object();  // injected as `self` before init()
    ...
    void serialize(nlohmann::json& json) const override {
        ...
        if (!properties.empty()) json["properties"] = properties;
    }
    ...
    void deserialize(const nlohmann::json& json) override {
        ...
        if (json.contains("properties"))     properties     = json["properties"];
        loaded        = false;
        errored       = false;
        m_updateAccum = 0.0f;
    }
